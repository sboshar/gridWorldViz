...
def wallGenerator(availablePos, minLength, maxLength, minNum, maxNum):
  ...
  for i in range(numWallGroup):
    ...
    length = random.randint(minLength, maxLength) #Depend on map size later?
...
def generateBoard(minWallLength, maxWallLength, minNumWalls, maxNumWalls, minR, maxR, minT, maxT):
  ...
  Args:
      availablePos (int): array of tuples of avialable positions
...
def setUpEnvironment(randomEnv=False, printEnv=False):
  ...
  Args:
      randomEnv (bool, optional): If true generates random environment. Defaults to False.
      printEnv (bool, optional): If true prints array representation of environment. Defaults to False.
...
def isValidState(s, b=np.array([[' ' for c in range(cols)] for r in range(rows)])):
  ...
  Args:
      s (tuple): state
      b (2d np array, optional): the state space of traps, walls, rewards, and blank spaces. Defaults to np.array([[' ' for c in range(cols)] for r in range(rows)]).
...
def plotQ(q_table, verbose=False, recolor=False, arrows=True):
  ...
  if recolor:
    ...
    newMin = minValue + (values[traps[0][0]][traps[0][1]] - minValue) * .02
    for t in traps:
      values[t[0]][t[1]] = newMin
    
  #colors the walls as the darkest color on the plot
  for w in walls:
    values[w[0]][w[1]] = newMin # 1, 0????
...
def value_iteration(q_table, eps=0.01, max_iter=10000, plot=False, plotEvery=False, verbose=False, recolor=False, arrows=True):
  ...
  Args:
      ...
      eps (float, optional): Defines the stopping criterion. Defaults to 0.01.
      max_iter (int, optional): The maximum iterations of value_iteration. Defaults to 10000.
      plot (bool, optional): Plots the value array, and policy using arrows in matplotlib. Defaults to False.
      plotEvery (bool, optional): Plots every iteration, click x to get next iteration. Defaults to False.
      verbose (bool, optional): print the delta between old and new q, and the array of values. Defaults to False.
  ...
  for i in range(max_iter):
    ...
    if plotEvery: plotQ(new_q_table, verbose=verbose, recolor=True, arrows=arrows)
    delta = 0  
...
if __name__ == "__main__":
  ...
  _ = value_iteration(q_table, plot=True, plotEvery=False, verbose=False, recolor=True)